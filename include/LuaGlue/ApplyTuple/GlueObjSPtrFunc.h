#ifndef LUAGLUE_APPLYTUPLE_GLUEOBJSPTRFUNC_H_GUARD
...
struct apply_glueobj_sptr_func
{
  ...
  static R applyTuple(LuaGlueBase *g, lua_State *s, LuaGlueTypeValue<std::shared_ptr<T>> &pObj,
                          ...
                          Args&&... args )
	{
		...
		return apply_glueobj_sptr_func<N-1>::applyTuple(g, s, pObj, f, std::forward<decltype(t)>(t), stack<ltype>::get(g, s, -(argCount-N+1)), std::forward<Args>(args)... );
	}
};
...
#endif /* LUAGLUE_APPLYTUPLE_GLUEOBJSPTRFUNC_H_GUARD */
